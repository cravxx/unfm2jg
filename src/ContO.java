        ...
        try
        {
            ...
            while((s = datainputstream.readLine()) != null) 
            {
                ...
                if(s1.startsWith("w"))
                {
                    ...
                    j++;
                    wheels.make(m, t, p, npl, (int)((float)getvalue("w", s1, 0) * f * f1 * nfmm_scale[0]), (int)((float)getvalue("w", s1, 1) * f * nfmm_scale[1]), (int)((float)getvalue("w", s1, 2) * f * nfmm_scale[2]), getvalue("w", s1, 3), (int)((float)getvalue("w", s1, 4) * f * f1), (int)((int)getvalue("w", s1, 5) * f), i1);
                    npl += 15;
                }
