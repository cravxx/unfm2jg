...
public class GameSparker extends Applet
    implements Runnable
{
	static final long serialVersionUID = 420L; /// like a class id; one is generated by default so we might as well set it as 420 AYYYY LMAO
	HashMap<String,Integer> properties;
	
    public boolean keyDown(Event event, int i) /// http://www.expandinghead.net/keycode.html
    {
        if(!exwist)
        {            
            if(i == 1004)
	...
	public int readcookie(String string) {
		...
		} catch (NoClassDefFoundError err) {// This block works when your Applet
											// is running inside a desktop Frame
			if (properties == null)
				return 17;
    ...
    public boolean keyUp(Event event, int i) /// http://www.expandinghead.net/keycode.html
    {
        if(!exwist)
        {                     
            if(i == 1004)
            ...
            Record record)
    {
        ...
        try
        {
			...
			final File file = new File(
					new StringBuilder().append("").append(stage_data).append(".txt").toString());
