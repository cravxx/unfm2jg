 ...
 *
 */
public class GameSparker extends JPanel implements FocusListener, KeyListener, MouseListener, MouseMotionListener {
	/**
	 * 
	 */
	private static final long serialVersionUID = -34048182014310663L;
	public static Utility utility;
    public static boolean applejava = System.getProperty("java.vendor").toLowerCase().contains("apple");
    ...
    private ContO acontos[];
    private ContO aconto1s[];
    private Madness amadnesses[];
    private float $f;
    private int $i1;
	...
	public GameSparker() {     
        ...
        offImage = new BufferedImage(670, 400, BufferedImage.TYPE_INT_RGB);
        if (offImage != null) {
        ...
        xtgraphics = new xtGraphics(medium, rd, this);
        try {
            xtgraphics.loaddata(k);
        } catch (IOException | URISyntaxException e) {
            JOptionPane.showMessageDialog(null, "Caught exception while loading data: " + e, "Fatal error", JOptionPane.ERROR_MESSAGE);
            e.printStackTrace();
        }
        record = new Record(medium);
        acontos = new ContO[carModels.length + trackModels.length + extraModels.length];
        loadbase(acontos, medium, trackers, xtgraphics);
        aconto1s = new ContO[3000];
        amadnesses = new Madness[7];
        int l = 0;
        do {
            amadnesses[l] = new Madness(medium, record, xtgraphics, l);         
...
//		else
    ...
    private void gameTick(Trackers trackers, Medium medium, CheckPoints checkpoints, xtGraphics xtgraphics, Record record, ContO[] aconto, ContO[] aconto1, Madness[] amadness) {
        Date date1 = new Date();
...
//                }
	...
	public void paintComponent(Graphics g) {
        gameTick(trackers, medium, checkpoints, xtgraphics, record, acontos, aconto1s, amadnesses);
